...
final class SwiftPackageManagerInteractorTests: TuistUnitTestCase {
    ...
    func test_install_when_shouldNotBeUpdated() throws {
        ...
        swiftPackageManagerGraphGenerator.generateStub = { path, automaticProductType, platforms, deploymentTargets, productSettings, swiftToolsVersion in
    ...
    func test_install_when_shouldNotBeUpdated_and_swiftToolsVersionPassed() throws {
        ...
        swiftPackageManagerGraphGenerator.generateStub = { path, automaticProductType, platforms, deploymentTargets, productSettings, swiftVersion in
            XCTAssertEqual(path, swiftPackageManagerBuildDirectory)
            XCTAssertEqual(automaticProductType, [:])
            XCTAssertEqual(platforms, [.iOS])
            XCTAssertEqual(deploymentTargets, [.iOS("13.0", [.iphone])])
